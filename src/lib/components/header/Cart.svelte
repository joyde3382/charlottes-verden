<script lang="ts">
	...
	import { toggleCart, removeFromCart, appStore } from "$lib/stores/cartStore";
    ...
    appStore.subscribe((newappStore) => {
  ...
  >
    ...
    <button
      ...
      on:click={toggleCart}
    >
    <span class="right-0 top-0 rounded-full bg-red-600 w-4 h-34 top right p-0 m-0 text-white font-mono text-sm  leading-tight text-center">{$appStore.cart.totalQuantity}
    </span>
    ...
    <div
      ...
      class="bg-neutral-600 absolute top-16 md:top-20 right-0 m-2 md:mx-0 min-h-[250px] rounded-lg shadow-lg leading-none flex flex-col transition-opacity duration-300 w-[min(calc(100vw-16px),380px)] 2xl:translate-x-1/2 2xl:right-1/2 2xl:top-[50px] z-50"
    >
          ...
          {#each $appStore.cart.products as product (product.id)}
            ...
            <div class="grid grid-cols-[auto_1fr_auto] grid-rows-2 items-center gap-x-4">
              <div class="w-14 rounded overflow-hidden col-span-1 col-start-1 row-span-2">
                <img src={product.thumbnail} alt="tesst" />
              </div>
              <span class="text-neutral-300 tracking-wide col-span-1 col-start-2 row-span-1 truncate">{product.title}</span
              >
              ...
              <button
                aria-label="Remove all {product.title} from cart"

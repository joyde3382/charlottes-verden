...
<div class="flex flex-col p-6 sm:px-0 sm:py-12 lg:p-0 gap-4 sm:gap-6 max-w-[480px] w-full mx-auto">
    ...
    <h1 class="font-bold text-3xl sm:text-5xl">{product.title}</h1>
    <p class="text-neutral-300 sm:text-lg sm:leading-snug">{product.title}</p>
    <!-- Price & Reduction -->
    <div class="flex items-center justify-between my-2 sm:my-4 font-bold lg:flex-col lg:items-start lg:gap-2">
      <div class="flex">
        <span class="text-3xl leading-[0.9] pb-0.5">${product.price}</span>
      </div>
    </div>
